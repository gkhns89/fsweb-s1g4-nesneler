...
function MenuElemaniOlustur(meal, value, category) {
  /*Kodlar buraya*/
  let mElement = { isim: meal, fiyat: value, kategori: category };
...
let kategori = "Pizzalar";

for (let i = 0; i < yiyecekler.length - 1; i += 2) {
  console.log(MenuElemaniOlustur(yiyecekler[i], yiyecekler[i + 1], kategori));
}
...
const burger = {
  ...
  indirim: function (kime) {
    let indirimliFiyat = Int32Array;
    if (kime === "öğretmen") {
      indirimliFiyat = this.fiyat * 0.75;
    } else if (kime === "öğrenci") {
      indirimliFiyat = this.fiyat * 0.75;
    } else if (kime === "diğer") {
...
*/
let kim =
  degerlendirmeler[degerlendirmeler.map((o) => o.isim).indexOf("Ahmet")]
    .geribildirim;
console.log(kim);
...
degerlendirmeler[
  degerlendirmeler.map((o) => o.isim).indexOf("Reyna")
].geribildirim = eksikVeri;

let kim2 =
  degerlendirmeler[degerlendirmeler.map((o) => o.isim).indexOf("Reyna")]
    .geribildirim;
console.log(kim2);
...
function DegerlendirmeEkle(degerlendirme, isim, puan, geribildirim) {
  ...
  degerlendirme.push(yeniDegerlendirme);
...
function AnahtardanDegerlendirmeAl(degerlendirmeDizisi, level) {
  /*Kodlar buraya*/
  let cumle =
    degerlendirmeDizisi[level].isim +
    " isimli kişi " +
    degerlendirmeDizisi[level].puan +
    " puan verdi ve şunları yazdı: " +
    degerlendirmeDizisi[level].geribildirim;
...
function SonDegerlendirmeyiAl(degerlendirmeDizisi) {
  ...
  let sonDegerlendirme =
    degerlendirmeDizisi[degerlendirmeDizisi.length - 1].isim +
    " isimli kişi " +
    degerlendirmeDizisi[degerlendirmeDizisi.length - 1].puan +
    " puan verdi ve şunları yazdı: " +
    degerlendirmeDizisi[degerlendirmeDizisi.length - 1].geribildirim;
